...
public class MainActivity extends AppCompatActivity {
    ...
    private RecyclerView.Adapter adapterTendencias, adapterGenero, adapterNuevosLanzamientos;
    private RecyclerView recyclerViewTendencias, recyclerViewGenero, recyclerViewNuevosLanzamientos;
    private ViewPager2 viewPager2;
    private Handler slideHandler = new Handler();
    private List<Videojuego> favoriteVideojuegoList = new ArrayList<>();
    ...
    private void initView() {
        ...
        imageViewFavorito.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                // Actualizar el estado del juego favorito
                favorito = !favorito;
                // Guardar el estado del juego favorito en las preferencias compartidas
    ...
    private void actualizarFavoritos() {
        // Filtrar los juegos favoritos de la lista principal
        List<Videojuego> favoritosTendencias = new ArrayList<>();
        List<Videojuego> favoritosNuevosLanzamientos = new ArrayList<>();
        for (Videojuego videojuego : videojuegoList) {
            if (sharedPreferences.getBoolean(videojuego.getTitulo() + "_favorito", false)) {
                favoritosTendencias.add(videojuego);
            }
        }
        ...
        for (Videojuego videojuego : proximoVideojuegoList) {
            if (sharedPreferences.getBoolean(videojuego.getTitulo() + "_favorito", false)) {
                favoritosNuevosLanzamientos.add(videojuego);
            }
        }
        // Actualizar el adaptador de nuevos lanzamientos con la lista de favoritos
        adapterNuevosLanzamientos = new VideojuegosAdapter(favoritosNuevosLanzamientos, MainActivity.this, listener);
        recyclerViewNuevosLanzamientos.setAdapter(adapterNuevosLanzamientos);
    }
